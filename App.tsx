...
// Define the global window.aistudio interface with correct modifiers to avoid TS conflict
declare global {
  interface Window {
    readonly aistudio: {
...
const App: React.FC = () => {
  ...
  useEffect(() => {
    if (mainRef.current) {
      // 在移动端，我们将视角定位在更中心的位置，考虑到屏幕较窄
      const initialX = window.innerWidth < 640 ? 2500 : 2400;
      const initialY = window.innerWidth < 640 ? 2100 : 2000;
      mainRef.current.scrollLeft = initialX;
      mainRef.current.scrollTop = initialY;
    }
  }, []);
  ...
  return (
    <div className="h-full w-full flex flex-col relative overflow-hidden bg-[#1a1a1a]">
      {/* Desktop Top Dock */}
      <nav 
        className={`hidden sm:flex fixed top-8 left-1/2 -translate-x-1/2 z-[5000] flex-col items-center space-y-4 transition-all duration-700 ${isNavVisible ? 'opacity-100 translate-y-0' : 'opacity-0 -translate-y-24 pointer-events-none'}`}
      >
        <div className="flex items-center space-x-2 p-2 bg-black/85 backdrop-blur-3xl border border-white/10 rounded-full shadow-2xl">
          {!isKeySettingsOpen && (
            <>
              <button onClick={() => setIsWizardOpen(true)} className="p-4 md:p-5 text-white/70 hover:text-white hover:bg-white/10 rounded-full transition-all" title={t.quickAdd}><Plus size={28}/></button>
              <button onClick={addNote} className="p-4 md:p-5 text-white/70 hover:text-white hover:bg-white/10 rounded-full transition-all" title="Add Sticky Note"><StickyNote size={24}/></button>
              <div className="w-[1px] h-8 bg-white/20 mx-1"></div>
              <button onClick={() => mainRef.current?.scrollTo({ left: 2400, top: 2000, behavior: 'smooth' })} className="p-4 md:p-5 text-white/70 hover:text-white hover:bg-white/10 rounded-full transition-all" title={t.resetView}><Focus size={24}/></button>
              <button onClick={() => setIsBackgroundModalOpen(true)} className="p-4 md:p-5 text-white/70 hover:text-white hover:bg-white/10 rounded-full transition-all" title={t.changeBackground}><ImageIcon size={24}/></button>
              <button onClick={() => { setIsLinking(!isLinking); setLinkStartId(null); }} className={`p-4 md:p-5 transition-all rounded-full flex items-center justify-center ${isLinking ? 'text-rose-500 bg-rose-500/20 scale-110' : 'text-white/70 hover:text-white hover:bg-white/10'}`} title="Connect Vision Nodes"><LinkIcon size={24}/></button>
              <button onClick={() => setIsAiGuideOpen(true)} className="p-4 md:p-5 text-white/70 hover:text-white hover:bg-white/10 rounded-full transition-all" title={t.aiGuide}><Sparkles size={24}/></button>
              <div className="w-[1px] h-8 bg-white/20 mx-1"></div>
            </>
          )}
          <button onClick={() => setIsKeySettingsOpen(!isKeySettingsOpen)} className={`p-4 md:p-5 rounded-full transition-all flex items-center justify-center ${isKeySettingsOpen ? 'bg-white text-stone-900 shadow-xl' : 'text-white/70 hover:text-white hover:bg-white/10'}`} title={t.apiSettings}>{isKeySettingsOpen ? <X size={24}/> : <Key size={24}/>}</button>
          <button onClick={() => setLang(lang === 'en' ? 'zh' : 'en')} className="p-4 md:p-5 text-white/70 hover:text-white hover:bg-white/10 rounded-full text-sm font-black tracking-tighter">{lang.toUpperCase()}</button>
        </div>
      ...
      {/* Mobile Bottom Dock */}
      <nav className="sm:hidden fixed bottom-6 left-1/2 -translate-x-1/2 z-[5000] w-[90%] max-w-[400px]">
        <div className="flex items-center justify-around p-3 bg-black/90 backdrop-blur-2xl border border-white/10 rounded-3xl shadow-2xl">
          <button onClick={() => setIsWizardOpen(true)} className="p-4 bg-stone-100 text-stone-900 rounded-2xl shadow-lg active:scale-95 transition-all"><Plus size={24}/></button>
          <button onClick={() => setShowMobileMenu(!showMobileMenu)} className={`p-4 rounded-2xl transition-all ${showMobileMenu ? 'bg-white/20 text-white' : 'text-white/60'}`}><Menu size={24}/></button>
          <button onClick={() => setIsAiGuideOpen(true)} className="p-4 text-white/60 active:text-white"><Sparkles size={24}/></button>
          <button onClick={() => mainRef.current?.scrollTo({ left: 2400, top: 2000, behavior: 'smooth' })} className="p-4 text-white/60 active:text-white"><Focus size={24}/></button>
        </div>

        {/* Mobile Expanded Menu Overlay */}
        {showMobileMenu && (
          <div className="absolute bottom-full left-0 right-0 mb-4 bg-black/95 backdrop-blur-3xl border border-white/10 rounded-[2rem] p-6 shadow-2xl animate-in slide-in-from-bottom-5 duration-300">
            <div className="grid grid-cols-2 gap-3 mb-6">
              <button onClick={() => { addNote(); setShowMobileMenu(false); }} className="flex flex-col items-center justify-center p-4 bg-white/5 rounded-2xl text-white/70 space-y-2"><StickyNote size={20}/><span className="text-[10px] font-bold uppercase tracking-widest">Note</span></button>
              <button onClick={() => { setIsBackgroundModalOpen(true); setShowMobileMenu(false); }} className="flex flex-col items-center justify-center p-4 bg-white/5 rounded-2xl text-white/70 space-y-2"><ImageIcon size={20}/><span className="text-[10px] font-bold uppercase tracking-widest">Canvas</span></button>
              <button onClick={() => { setIsLinking(!isLinking); setShowMobileMenu(false); }} className={`flex flex-col items-center justify-center p-4 rounded-2xl space-y-2 ${isLinking ? 'bg-rose-500/20 text-rose-500' : 'bg-white/5 text-white/70'}`}><LinkIcon size={20}/><span className="text-[10px] font-bold uppercase tracking-widest">Link</span></button>
              <button onClick={() => { setIsKeySettingsOpen(!isKeySettingsOpen); setShowMobileMenu(false); }} className="flex flex-col items-center justify-center p-4 bg-white/5 rounded-2xl text-white/70 space-y-2"><Key size={20}/><span className="text-[10px] font-bold uppercase tracking-widest">API</span></button>
            </div>
            <div className="flex items-center justify-between border-t border-white/10 pt-4 px-2">
               <button onClick={() => { setLang(lang === 'en' ? 'zh' : 'en'); setShowMobileMenu(false); }} className="text-white/40 text-xs font-black tracking-widest">{lang.toUpperCase()}</button>
               <button onClick={() => setShowMobileMenu(false)} className="text-white/40"><X size={20}/></button>
            </div>
          </div>
        )}
      </nav>
      ...
      {isKeySettingsOpen ? (
        <div className="flex-1 bg-stone-950 flex flex-col items-center justify-center p-6 sm:p-12 animate-in fade-in duration-1000 text-center space-y-8 sm:space-y-12">
           ...
           <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6 max-w-2xl w-full">
              <button onClick={async () => { await window.aistudio.openSelectKey(); setIsKeySettingsOpen(false); }} className="flex flex-col items-center justify-center p-8 sm:p-10 bg-white rounded-3xl sm:rounded-[2.5rem] hover:bg-stone-50 transition-all group space-y-4 shadow-2xl"><Zap size={20} className="text-stone-900" /><span className="text-[10px] font-black uppercase tracking-widest text-stone-900">{t.configureKey}</span></button>
              <a href="https://ai.google.dev/gemini-api/docs/billing" target="_blank" className="flex flex-col items-center justify-center p-8 sm:p-10 bg-white/5 border border-white/10 rounded-3xl sm:rounded-[2.5rem] hover:bg-white/10 transition-all group space-y-4"><ExternalLink size={20} className="text-white/70" /><span className="text-[10px] font-black uppercase tracking-widest text-white/70">{t.billingDocs}</span></a>
           </div>
        ...
        {editingItem && (
          <div className="max-w-xl mx-auto space-y-8 relative">
            <div className="space-y-6">
                ...
                <div className="space-y-8">
                  <div className="space-y-3">
                    <div className="flex items-center justify-between">
                      ...
                      <button type="button" onClick={() => setShowFontBrowser('zh')} className="text-stone-400 hover:text-stone-900 transition-colors"><Plus size={14} /></button>
                    </div>
                    <div className="flex flex-wrap gap-2">
                      {allZhFonts.map(f => (
                        <button key={f.value} onClick={() => setEditingItem({ ...editingItem, fontFamily: f.value })} style={{ fontFamily: f.value }} className={`px-4 py-2 rounded-xl border text-sm transition-all ${editingItem.fontFamily === f.value ? 'bg-stone-900 border-stone-900 text-white shadow-lg' : 'bg-stone-50 hover:bg-stone-100'}`}>{f.name}</button>
                      ))}
                    </div>
                  </div>
                  <div className="space-y-3">
                    <div className="flex items-center justify-between">
                      ...
                      <button type="button" onClick={() => setShowFontBrowser('en')} className="text-stone-400 hover:text-stone-900 transition-colors"><Plus size={14} /></button>
                    </div>
                    <div className="flex flex-wrap gap-2">
                      {allEnFonts.map(f => (
                        <button key={f.value} onClick={() => setEditingItem({ ...editingItem, fontFamily: f.value })} style={{ fontFamily: f.value }} className={`px-4 py-2 rounded-xl border text-sm transition-all ${editingItem.fontFamily === f.value ? 'bg-stone-900 border-stone-900 text-white shadow-lg' : 'bg-stone-50 hover:bg-stone-100'}`}>{f.name}</button>
              ...
              <div className="space-y-4">
                <input value={editingItem.title} onChange={e => setEditingItem({ ...editingItem, title: e.target.value })} className="w-full bg-stone-50 border-stone-100 border p-5 rounded-2xl text-xl focus:ring-1 focus:ring-stone-400 transition-all" style={{ fontFamily: editingItem.fontFamily }} />
                <textarea value={editingItem.affirmation} onChange={e => setEditingItem({ ...editingItem, affirmation: e.target.value })} className="w-full bg-stone-50 border-stone-100 border p-5 rounded-2xl h-32 focus:ring-1 focus:ring-stone-400 transition-all text-base" style={{ fontFamily: editingItem.fontFamily }} />
              </div>
            </div>
                ...
                <div className="flex items-center justify-between p-6 sm:p-8 border-b border-stone-50">
                   ...
                   <div className="flex items-center space-x-2 sm:space-x-4">
                      <button 
                        ...
                        className="p-3 bg-stone-100 text-stone-900 rounded-full hover:bg-stone-200 transition-all active:scale-95"
                        title={t.shuffleFonts}
                      >
                        <RefreshCw size={16} />
                      </button>
                      <button onClick={() => setShowFontBrowser(null)} className="p-3 text-stone-300 hover:text-stone-900"><X size={24} /></button>
                   </div>
                </div>
                <div className="flex-1 overflow-y-auto p-6 sm:p-8 custom-scrollbar">
                   <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      {visibleFonts.map(f => (
                        <button key={f.name} onClick={() => selectFontFromBrowser(f.name, showFontBrowser)} className="p-5 sm:p-6 bg-stone-50 hover:bg-stone-900 text-stone-900 hover:text-white rounded-2xl border border-stone-100 flex items-center justify-between group transition-all text-left">
      ...
      <Modal isOpen={isBackgroundModalOpen} onClose={() => setIsBackgroundModalOpen(false)} title={t.changeBackground}>
        <div className="grid grid-cols-1 xs:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
          <button onClick={() => bgInputRef.current?.click()} className="h-40 sm:h-48 rounded-[1.5rem] sm:rounded-[2rem] border-4 border-dashed border-stone-200 flex flex-col items-center justify-center space-y-3 hover:border-stone-900 transition-all group">
